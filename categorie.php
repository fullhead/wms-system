...
<?php
    if (isset($_POST['add_cat'])) {
        ...
        if (empty($errors)) {
            ...
            $sql .= " VALUES ('{$cat_name}')";
            if ($db->query($sql)) {
                $session->msg("s", "Успешно добавлена Новая категория");
                redirect('categorie.php', false);
            } else {
                $session->msg("d", "Извините, не удалось добавить!");
                redirect('categorie.php', false);
            }
        } else {
            ...
            <div class="panel-body">
                <form method="post" action="categorie.php">
                    <div class="form-group">
                        <input type="text" class="form-control" name="categorie-name" placeholder="Название категории">
                    </div>
                    <button type="submit" name="add_cat" class="btn btn-primary">Добавить категорию</button>
                </form>
            </div>
        </div>
    </div>
