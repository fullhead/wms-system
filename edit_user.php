...
<?php
    ...
    if (!$e_user) {
        $session->msg("d", "Отсутствует ID пользователя.");
        redirect('users.php');
    }
?>
    ...
    if (isset($_POST['update'])) {
        ...
        if (empty($errors)) {
            ...
            if ($result && $db->affected_rows() === 1) {
                $session->msg('s', "Аккаунт успешно обновлен!");
                redirect('edit_user.php?id=' . (int)$e_user['id'], false);
            } else {
                $session->msg('d', ' Ошибка при обновление аккаунта!');
                redirect('edit_user.php?id=' . (int)$e_user['id'], false);
            }
        } else {
...
<?php
    ...
    if (isset($_POST['update-pass'])) {
        ...
        if (empty($errors)) {
            ...
            $result = $db->query($sql);
            if ($result && $db->affected_rows() === 1) {
                $session->msg('s', "Пароль у пользователя успешно обновлен!");
                redirect('edit_user.php?id=' . (int)$e_user['id'], false);
            } else {
                $session->msg('d', ' Ошибка при обновление пароля пользователя!');
                redirect('edit_user.php?id=' . (int)$e_user['id'], false);
            }
        } else {
            $session->msg("d", $errors);
        ...
        <div class="panel panel-default">
            <div class="panel-heading">
                <strong>
                    ...
                    Изменить пользователя: <?php echo remove_junk(ucwords($e_user['name'])); ?>
                </strong>
            </div>
    ...
    <div class="col-md-6">
        <div class="panel panel-default">
            <div class="panel-heading">
                <strong>
                    <span class="glyphicon glyphicon-th"></span>
                    Изменить пароль для: <?php echo remove_junk(ucwords($e_user['name'])); ?>
                </strong>
            </div>
            <div class="panel-body">
                <form action="edit_user.php?id=<?php echo (int)$e_user['id']; ?>" method="post" class="clearfix">
                    <div class="form-group">
                        ...
                        <input type="password" class="form-control" name="password"
                               placeholder="Введите новый пароль для пользователя">
                    </div>
                    <div class="form-group clearfix">
                        <button type="submit" name="update-pass" class="btn btn-danger pull-right">Обновить</button>
                    </div>
                </form>
            </div>
        </div>
